...
async function checkin(cookie: string) {
  ...
  const text = await r.text();
  let data: any;
  try { data = JSON.parse(text); } catch { throw new Error(`Checkin HTTP ${r.status}: ${text.slice(0, 200)}`); }
  return data;
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ ok: true, ret: result.ret, msg: result.msg, traffic: result.traffic });
  } catch (err) {
