...
async function fetchJobDetail(url: string): Promise<string> {
  try {
    ...
    if (!res.ok) return "";
    const html = await res.text();
    // 移除 style 和 script 标签及其内容，防止 CSS/JS 泄露到文本中
    const cleaned = html
      .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "")
      .replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "");
    // 提取 div.text 中的任务描述内容
...
export async function GET() {
  try {
    ...
    const seenData = await redisCommand("get", "r5:latest_id");
    ...
    for (const job of newJobs) {
      job.description = await fetchJobDetail(job.url);
    }
    ...
    await redisCommand("set", "r5:latest_id", String(newLatestId));
